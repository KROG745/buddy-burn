            ...
            {getWeekDays().map((day, index) => {
              ...
              return (
                <Card 
                  key={index}
                  className={cn(
                    "p-3 md:p-4 min-h-[140px] md:min-h-[180px] lg:min-h-[220px] transition-all duration-200 cursor-pointer hover:shadow-md flex flex-col",
                  ...
                  <div className="space-y-2 flex-1 overflow-y-auto">
                    {dayWorkouts.length === 0 ? (
                      ...
                      dayWorkouts.slice(0, 3).map((workout) => (
                        <div
                          key={workout.id}
                          className="p-2 md:p-3 bg-card rounded-lg border border-border shadow-sm"
                        >
                          <div className="font-medium text-sm md:text-base text-foreground leading-tight mb-1">
                            {workout.title}
                          </div>
                          <div className="flex items-center gap-1.5 text-muted-foreground text-xs md:text-sm mb-1.5">
                            <Clock className="w-3.5 h-3.5 flex-shrink-0" />
                            <span>{workout.time}</span>
                          </div>
                          <Badge 
                            variant="outline" 
                            className={cn("text-xs px-2 py-0.5", getIntensityColor(workout.intensity))}
                          >
                            {workout.intensity}
                          </Badge>
                        </div>
                    ...
                    {dayWorkouts.length > 3 && (
                      <div className="text-sm text-muted-foreground text-center py-1 font-medium">
